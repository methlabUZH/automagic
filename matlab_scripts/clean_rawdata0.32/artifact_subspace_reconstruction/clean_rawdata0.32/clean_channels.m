...
chanCorr_channels = false(C,1);
removed_channels(usable_channels) = sum(flagged,2)*window_len > max_broken_time;
chanCorr_channels(usable_channels) = sum(flagged,2)*window_len > max_broken_time;
...
elseif any(removed_channels)
    ...
    signal.etc.ChannelCriterion = chanCorr_channels;
    signal.etc.LineNoiseCriterion = noise_mask';
end
